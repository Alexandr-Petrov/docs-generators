...
# Usage: ./generate-invoice.sh input.md [output.pdf]
...
if [ $# -lt 1 ]; then
    ...
    echo "Example: $0 examples/sample-invoice.md"
    exit 1
fi
...
echo -e "${GREEN}Generating Invoice...${NC}"
echo "Input:    $INPUT_FILE"
echo "Output:   $OUTPUT_FILE"
echo "Template: $TEMPLATE_DIR/invoice-template.latex"

# Generate PDF
pandoc "$INPUT_FILE" \
    --template="$TEMPLATE_DIR/invoice-template.latex" \
    --pdf-engine=pdflatex \
    -o "$OUTPUT_FILE"

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Invoice generated successfully: $OUTPUT_FILE${NC}"
    ...
    echo -e "${RED}✗ Error generating invoice${NC}"
    exit 1
fi
